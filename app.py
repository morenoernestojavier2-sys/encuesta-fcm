...
# --- DISEÑO VISUAL BLINDADO (Alto contraste, Bordes negros y Sombras) ---
st.markdown("""
<style>
    .stApp {
...
def registrar_movimiento_doble(seccion_origen, accion_realizada):
    correo_actual = st.session_state.respuestas.get("Email", "SIN_EMAIL_REGISTRADO")
    datos = {
...
if st.session_state.modo_admin:
    ...
    try:
        ...
        if res_respuestas: df = pd.DataFrame(res_respuestas)
    except: pass

    try:
        res_movimientos = requests.get(f"{URL_DE_TU_GOOGLE_SCRIPT}?sheet=Movimientos").json()
        if res_movimientos: df_hist = pd.DataFrame(res_movimientos)
    except: pass
        
    if os.path.isfile('Base_Respuestas.csv'):
        df_local = pd.read_csv('Base_Respuestas.csv')
        if df.empty: df = df_local
        else: df = pd.concat([df, df_local]).drop_duplicates(subset=['Email', 'Fecha'], keep='last').reset_index(drop=True)
        
    if os.path.isfile('Historial_Movimientos.csv'):
        df_hist_local = pd.read_csv('Historial_Movimientos.csv')
        if df_hist.empty: df_hist = df_hist_local
        else: df_hist = pd.concat([df_hist, df_hist_local]).drop_duplicates().reset_index(drop=True)
    ...
    if not df.empty:
        ...
        st.warning("Aún no hay respuestas guardadas en el sistema para graficar.")

else:
    # --- MODO ALUMNO (Navegación de las 7 Secciones) ---
    ...
    if st.session_state.seccion == 1:
        ...
        st.write("---")
        if st.button("Siguiente ➡️", key="btn_s1"):
        ...
        with col1:
            if st.button("⬅️ Atrás", key="back_s2"):
                st.session_state.seccion = 1
                st.rerun()
        with col2:
            if st.button("Siguiente ➡️", key="btn_s2"):
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("⬅️ Atrás", key="back_s3"):
                st.session_state.seccion = 2
                st.rerun()
        with col2:
            if st.button("Siguiente ➡️", key="btn_s3"):
        ...
        necesarias = st.select_slider("", options=list(range(11)), value=5, key="necesarias_slider")
        ...
        confianza = st.select_slider(" ", options=[1,2,3,4,5], value=3, key="confianza_slider")

        st.write("---")
        col1, col2 = st.columns(2)
        with col1:
            if st.button("⬅️ Atrás", key="back_s4"):
                st.session_state.seccion = 3
                st.rerun()
        with col2:
            if st.button("Siguiente ➡️", key="btn_s4"):
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("⬅️ Atrás", key="back_s5"):
                st.session_state.seccion = 4
                st.rerun()
        with col2:
            if st.button("Siguiente ➡️", key="btn_s5"):
        ...
        st.write("---")
        if st.button("Volver al inicio (Nueva respuesta)", key="btn_restart"):
